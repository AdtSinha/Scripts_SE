...
const { phoneNumnerKeyMapper } = require('./phoneNumberMapper');
...
const modifyPhoneNumbers = (baseCountry, phoneNumberMapper) => {
    return baseCountry.map(item => {
        const { _id, ...rest } = item;
        const correspondingItem = phoneNumberMapper.find(element => element.phoneNumber === rest.phoneNumber);
...
const insertModifiedPhoneNumbers = async (getModifiedBaseCountry, client) => {
  try {
    const database = client.db('commonDB');
    const collection = database.collection('phoneNumbers');
    ...
    console.error('Error inserting modified phoneNumbers :', error);
    return 0;
  }
};
...
const getPhoneNumbers = async () => {
  ...
  try {
    ...
    const phoneNumbersMapper = phoneNumnerKeyMapper;
    const getModifiedPhoneNumbers = modifyPhoneNumbers(result, phoneNumbersMapper);
    const insertModifiedResult = await insertModifiedPhoneNumbers(getModifiedPhoneNumbers, client);
    return insertModifiedResult;
  } catch (error) {
    console.error('Error getting phoneNumbers :', error);
    return 0;
  } finally {
    if (client) {
      await client.close();
